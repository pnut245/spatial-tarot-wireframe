...
function selectedIndexFromRotation(rotationDeg, cardCount) {
  ...
  const normalized = normDeg(360 - normDeg(rotationDeg));
...
function renderWheel({ mount, cards, rotationDeg, activeIndex }) {
  ...
  for (let i = 0; i < cards.length; i += 1) {
    ...
    svgParts.push(`<path class="${cls}" data-idx="${i}" d="${path}" fill="${palette(i)}"></path>`);
    svgParts.push(
  ...
  if (wheel) wheel.style.transform = `rotate(${rotationDeg}deg)`;
}
...
function main() {
  ...
  const state = { rotationDeg: 0, activeIndex: 0, spinning: false };
